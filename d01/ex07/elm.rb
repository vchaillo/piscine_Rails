...
def get_color(elem)
	color = "l_orange" if [1, 6, 7, 8, 15, 16, 34].include?(elem[:number].to_i)
	color = "red" if [2, 10, 18, 36, 54, 86, 118].include?(elem[:number].to_i)
	color = "purple" if [3, 11, 19, 37, 55, 87].include?(elem[:number].to_i)
	color = "blue" if [4, 12, 20, 38, 56, 88].include?(elem[:number].to_i)
	color = "yellow" if [5, 14, 32, 33, 51, 52].include?(elem[:number].to_i)
	color = "orange" if [9, 17, 35, 53, 85, 117].include?(elem[:number].to_i)
	color = "green" if [13, 31, 49, 50, 81, 82, 83, 84, 113, 114, 115, 116].include?(elem[:number].to_i)
	color = "l_blue" if (Array(21..30) + Array(39..48) + Array(72..80) + Array(104..112)).include?(elem[:number].to_i)
	return color
end
...
def create_case(file, elem)
	...
	file << "		</li>\n"
	file << "		<li class=\"Name\">\n"
	file << "			<h4>#{elem[:name]}</h4>\n"
	file << "		</li>\n"
	file << "		<li class=\"Z\">\n"
	file << "			#{elem[:number]}\n"
	file << "		</li>\n"
	file << "		<li class=\"e\">\n"
	file << "			#{elem[:electrons].split.map(&:to_i).inject(0, :+)} e\n"
	file << "		</li>\n"
	file << "	</ul>\n"
	file << "</td>\n"
end
